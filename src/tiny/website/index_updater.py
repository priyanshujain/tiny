...
class IndexUpdater:
        ...
        try:
            ...
            try:
                ...
                updated_content = self._insert_entry(
                    current_content, new_entry, blog_content.date
                )
                ...
                if self.file_manager.validate_react_component(self.index_path):
                    ...
                    logger.info(
                        f"Successfully added entry to index: {blog_content.title}"
                    )
                    return True
                else:
                    ...
                    logger.error(
                        "Updated index file failed validation, restored backup"
                    )
                    return False
    ...
    def _insert_entry(self, content: str, new_entry: str, entry_date: str) -> str:
        ...
        writings_section_pattern = (
            r'(<div className="mt10 ma20-l">)(.*?)(\s*</div>\s*</div>\s*</div>)'
        )
    ...
    def _fallback_insert(self, content: str, new_entry: str) -> str:
        ...
        logger.error(
            "Could not automatically insert entry, manual intervention required"
        )
        return content
